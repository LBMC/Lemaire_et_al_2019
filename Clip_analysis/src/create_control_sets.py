...
def make_control_sets(nb_iteration, hg19_chromosome_file, fasterdb_bed, exon_type, test_coordinates, fasta_dic):
    ...
    if not os.path.isfile(output_file):
        dic_chrom_size = create_sequence_dic(hg19_chromosome_file)
        print(fasterdb_bed)
        list_coordinates = load_fasterDB_coordinates(fasterdb_bed, exon_type)
        print(list_coordinates[0:5])
        list_size = get_len_list(test_coordinates)
        nb_control = 100000
        # nb_control = 5
        print("Getting control frequencies")
        new_coordinates = create_control_coordinates(list_coordinates, nb_control, list_size, dic_chrom_size)
        prind("new coordinates : \n ---> %s" % str(new_coordinates))
        dic_frequency = get_sequences_frequencies(new_coordinates, fasta_dic)
    ...
    # dic_frequencies = control_dictionaries_generator(dic_frequency, 2, 3)
