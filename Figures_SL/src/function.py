...
def ag_dinucleotide_calculator(sequence):
    """
...
def svm_bp_finder_launcher(svm_bp_launcher, input_file, exon, size):
    ...
    if not df.empty:
        ...
        if float(df["svm_scr"]) > 0:
            ...
            bp = int(df["ss_dist"])
            ag_count = ag_dinucleotide_calculator(seq[-bp+1:])
        ...
        return bp_score, ppt_score, nb_putative_bp, nb_good_bp, sequence, ag_count, hbound
    else:
...
def bp_ppt_calculator(exon_list, size=100):
    ...
    return bp_score_list, ppt_score_list, nb_bp_list, nb_good_bp_list, sequence_list, ag_count_list, hbound_list
