...
out_dir=./SFs_gathGCbiasCline_FLtab
mkdir -vp ${out_dir} >&2
echo ">>> ${out_dir}" >&2
for exon_list in $( find ./SFs_gathGCbiasCline/* -maxdepth 0 -name "*.txt" ); do
  echo "$exon_list" >&2
...
out_dir=./SFs_gathGCbias_FLtab
mkdir -vp ${out_dir} >&2
echo ">>> ${out_dir}" >&2
for exon_list in $( find ./SFs_gathGCbias/* -maxdepth 0 -name "*.txt" ); do
  echo "$exon_list" >&2
  exon_list_name=$( basename $exon_list .txt )
  Rscript ${R_script} ${exon_list} > ${out_dir}/${exon_list_name}.tab
done
...
mkdir -vp ${out_dir} >&2
echo ">>> ${out_dir}" >&2
for exon_list in $( find ./SFs_seb/* -maxdepth 0 -name "down*.txt" ); do
  echo "$exon_list" >&2
...
out_dir=./ref_list_FLtab
mkdir -vp ${out_dir} >&2
echo ">>> ${out_dir}" >&2
for exon_list in $( find ./ref_list/* -maxdepth 0 -name "*.txt" ); do
  echo "$exon_list" >&2
  exon_list_name=$( basename $exon_list .txt )
  Rscript ${R_script} ${exon_list} > ${out_dir}/${exon_list_name}.tab
done
