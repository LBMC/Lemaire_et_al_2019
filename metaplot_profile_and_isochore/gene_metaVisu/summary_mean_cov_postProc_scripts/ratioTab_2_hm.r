...
hm_col <- colorRampPalette( c( "blue", "white", "red" ) )( n = 100 )

col_breaks = c(
  head( seq( min( c( mat_val, -0.1 ), na.rm=T ), 0, length=51 ), n=-1 ),
  0,
  tail( seq( 0, max( c( mat_val, 0.1 ), na.rm=T ), length=51 ), n=-1 )
)

png( out_png, heigh=1200, width=1200 )
# par( mar= c( 5, 5, 5, 5 ) )
heatmap.2(
  mat_val,
  density.info="none",  # turns off density plot inside color legend
