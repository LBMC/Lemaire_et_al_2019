...
for measure in smc_h[ 0:5 ]:
    ...
    for samp in samp_list:
        ...
        for ratio_idx, ratio in enumerate( ratio_list ):
            nume = float( [ xxx[ meas_idx ] for xxx in smc_lines_sub if xxx[ annot_idx ] == ratio[ 0 ] ][ 0 ] )
            denom = float( [ xxx[ meas_idx ] for xxx in smc_lines_sub if xxx[ annot_idx ] == ratio[ 1 ] ][ 0 ] )
