...
base_fraction_metagene () {
  ...
  local ssRegion_ext_dir=$5 # results_psi_FDB/splicing_site_regions/inEx50_outOff100/win1b
  local min_reads=$6 # minimal number of aligned reads on a CpG to considered it in mCpG_metagene
  ...
  local bf_sel_part='--bf-all'
  flag='--bf-parts'
  if [[ "x${@}" == x*"${flag}"* ]]; then
    bf_sel_part="$( echo ${@} | awk -F "${flag} " '{ print $2 }' | cut -d ' ' -f 1 )"
  fi
  ...
  local ssRegion_ext_ref_list=''
  if [[ "x${@}" == x*"--ex-tab-ref"* ]]; then
  ...
  if [[ "x${@}" == x*"--with-refs-exons"* ]]; then
    ssRegion_ext_list="$( echo ${ssRegion_ext_ref_list} ${ssRegion_ext_list} | tr ' ' ',' )"
  fi
  ...
  "
  if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--dna-seq"* ]]; then
    echo "$cmd"
    eval "$cmd"
  fi


  prefixes_list=$( echo ${prefixes_ref} ${prefixes} | tr ' ' ',' )
  seq_files_all=()
  fasta_files_all=()
  ...
  "
  if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--seq-to-fasta"* ]]; then
    echo "$cmd"
    eval "$cmd"
  fi
  ...
  "
  if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--base-metaplot"* ]]; then
    echo "$cmd"
    eval "$cmd"
  fi
  ...
  cmd="
  weblogo_plots_script $( echo ${seq_files_all[*]} | tr ' ' ',' ) ${fig_dir} ${SS} ${metagene_param} ${prefixes_list} \
    --nbr-cores $NBRCORES
  "
  if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--weblogo"* ]]; then
    echo "$cmd"
    eval "$cmd"
  fi
  ...
  "
  if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--rna-folding"* ]]; then
    echo "$cmd"
    eval "$cmd"
  fi
  ...
  for expSeq_source in $( find ${base_fraction_metagene_dir}/methylation_metagene/coverage_bank/ -name "*_script.sh" | sort ); do # | grep WGBS ); do
    ...
    for (( spl_id=0; spl_id<${#bam_array[*]}; spl_id++ )); do
      ...
      "
      if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--mcpg-metaplot"* || "x$bf_sel_part" == x*"--mcpg-extract"* ]]; then
        echo "$cmd"
        eval "$cmd"
      fi
    done
    ...
    methylation_metagene_script $( echo ${seq_files_all[*]} | tr ' ' ',' ) $( echo ${all_meth_file_array[*]} | tr ' ' ',' ) ${out_meth_meta_dir} ${SS} ${metagene_param} ${prefixes_list} ${expSeq_cond} ${expSeq_rep} ${comp_pair} ${min_reads} \
      --nbr-cores $NBRCORES --fig-prefix ${fig_prefix} ${color_pallette}
    "
    if [[ "x$bf_sel_part" == x*"--bf-all"* || "x$bf_sel_part" == x*"--mcpg-metaplot"* || "x$bf_sel_part" == x*"--mcpg-metaplot-fig"* ]]; then
      echo "$cmd"
      eval "$cmd"
    fi
