...
meth_acf_plot_dir <- dirname(sys.frame(1)$ofile)
source( paste( meth_acf_plot_dir, 'base_fraction_metagene_fig_name.r', sep='/' ) )
source( paste( meth_acf_plot_dir, 'base_colors.r', sep='/' ) )
source( paste( meth_acf_plot_dir, 'sub_col_pal.r', sep='/' ) )

## build the curves of metagenes
meth_acf_plot_fc <- function( list_per_meta, prefix, vec_annot_name, color_pallette=NULL, out_dir_pref='autoCorrelation', suffix=NULL ) {
  ...
  out_prefix <- paste0( acf_dir, '/', unlist( strsplit( basename( metagene_fig_name ), split='.', fixed=TRUE ) )[ 1 ] )
  if ( ! is.null( suffix ) ) {
