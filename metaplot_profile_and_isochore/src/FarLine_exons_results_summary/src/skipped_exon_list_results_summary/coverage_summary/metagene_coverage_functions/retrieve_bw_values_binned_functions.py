...
def retrieve_multi_interval( bed, bw, nbins, group_concat=False, mean_val=False, start_left=0, end_right=-1 ):
    if not group_concat:
        ...
        for group_idx, group in enumerate( group_list ):
            ...
            inter = retrieve_grouped_val_inter( sub_tab, bw, nbins=nbins )

            if end_right == -1:
                end_right = len( inter ) - 1
                pass
            inter = inter[ start_left:end_right ]
            all_inter[ group_idx ] = inter
            pass

    if mean_val:
        all_inter = [ sum( inter ) / len( inter ) for inter in all_inter ]
        pass

    return( all_inter )


#~ def retrieve_multi_mean_cov( bed, bw, nbins, group_concat ):
...
#~         for annot_idx, annot in enumerate( bed ):
#~             # print( str( annot_idx ) + '/' + str( len( bed ) ), end='\r', file=sys.stderr )
#~             inter = retrieve_val_inter( annot, bw, nbins=nbins )
#~             all_means[ annot_idx ] = sum( inter ) / len( inter )
#~             pass
#~     else:
#~         bed = [ xxx + xxx[ 3 ].split( '_' ) for xxx in bed ]
