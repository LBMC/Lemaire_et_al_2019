...
wdir=${HOME}/analyses_SEBASTIEN/analyse_RNA-Seq_splicingFactors_2018-07/gene_metaVisu/res_jbc
...
col_nbr=0
target_mem=''
sf_list="$( ls geneReg_jbc/*_down_exon.bed | awk -F '/' '{ print $NF }' | awk -F '_down' '{ print $1 }' | sed s%'_\(HEK293\|HepG2\|K562\|MCF7\|HeLa\)'%''% | sort | uniq | rev | tr '\n' ' ' | rev | less )"
for mysf in ${sf_list}; do
  echo "mysf: $mysf"
  report_signature="${mysf}"
  ...
  for signal_group in ${signal_group_list}; do #histoneAc_clineChromLore histoneMeth_complete_refined
    ...
    while read signal; do
      ...
      vt_key=${annot_type}
        ...
        cmd="find ${wdir}/${mysf}_jbc*/geneVisu_${annot_type}/results/figures/coverage_summary/${visu_type[${vt_key}]}/${signal}/cov/metagene_${measure_type}/ -name \"metagene_${measure_type}*.png\""
        # echo ">>> $cmd"
      ...
      # echo "!!! SIGNAL: $signal"
      out_img=${out_pertrack}/${mysf}_${signal}${marq}.png
