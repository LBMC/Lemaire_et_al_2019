...
for measure_idx, measure in enumerate( measure_list ):
    ...
    for res_dir in res_dir_list:
        exon_list_pattern = res_dir + '/' + feature_dir + '/*_3SS_' + measure + '_*.png'
        exon_list_dic[ res_dir ] = sorted( [ xxx.split( 'S_' + measure + '_' )[ -1 ].split( '.' )[ 0 ] for xxx in glob.glob( exon_list_pattern ) ] )
        pass
    ...
    for exon_idx in range( exon_list_nb ):
        ...
        if measure != '-':
            ...
            for res_dir_idx, res_dir in enumerate( res_dir_list ):
                ...
                current_pane_x = current_x + res_dir_idx * ( offset_x + siGL2_signal_img_height * 2 )
                glob_pattern = res_dir + '/' + feature_dir + '/*_3SS_' + measure + '_' + exon + '.png'
                ...
                glob_pattern = res_dir + '/' + feature_dir + '/*_5SS_' + measure + '_' + exon + '.png'
                image_file = glob.glob( glob_pattern )[ 0 ]
                img_pane_fc( image_file, slide, current_pane_x, current_y, siGL2_signal_img_height )
                pass
            pass
        ...
        if measure == '-' or ( nb_meas % nb_lines == nb_lines - 1 and exon_idx != exon_list_nb - 1 ):
            # create the slide
            slide_layout = prs.slide_layouts[ 6 ]
            slide = prs.slides.add_slide( slide_layout )
            new_slide = True
