...
for measure_idx, measure in enumerate( measure_list ):
    ...
    if measure != '-':
        ...
        for res_dir_idx, res_dir in enumerate( res_dir_list ):
            if center_meas:
                current_pane_x = current_x + res_dir_idx * ( offset_x + siGL2_signal_img_height * 2 )
                glob_pattern = res_dir + '/' + feature_dir + '/' + measure + '/cov/metagene_' + measure_type + '/metagene_' + measure_type + '_*_center.png'
                image_file = get_path( glob_pattern ) #glob.glob( glob_pattern )[ 0 ]
                img_pane_fc( image_file, slide, current_pane_x, current_y, siGL2_signal_img_height )
                current_pane_x = current_x + res_dir_idx * ( offset_x + siGL2_signal_img_height * 2 )
                pass
            else:
                current_pane_x = current_x + res_dir_idx * ( offset_x + siGL2_signal_img_height * 2 )
                glob_pattern = res_dir + '/' + feature_dir + '/' + measure + '/cov/metagene_' + measure_type + '/metagene_' + measure_type + '_*_3SS.png'
                image_file = get_path( glob_pattern ) #glob.glob( glob_pattern )[ 0 ]
                img_pane_fc( image_file, slide, current_pane_x, current_y, siGL2_signal_img_height )

                current_pane_x = current_pane_x + siGL2_signal_img_height
                glob_pattern = res_dir + '/' + feature_dir + '/' + measure + '/cov/metagene_' + measure_type + '/metagene_' + measure_type + '_*_5SS.png'
                image_file = get_path( glob_pattern ) #glob.glob( glob_pattern )[ 0 ]
                img_pane_fc( image_file, slide, current_pane_x, current_y, siGL2_signal_img_height )
                pass
        pass

    if measure == '-' or ( nb_meas % 4 == 3 and measure_idx != len( measure_list ) - 1 ):
        # create the slide
        slide_layout = prs.slide_layouts[ 6 ]
        slide = prs.slides.add_slide( slide_layout )
        new_slide = True
