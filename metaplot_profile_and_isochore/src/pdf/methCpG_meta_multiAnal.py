...
for measure_idx, measure in enumerate( measure_list ):
    ## recover the conditions of the experiment
    cond_list_dic = {}
    for res_dir in res_dir_list:
        cond_list_pattern = res_dir + '/' + feature_dir + '/' + measure + '/*_3SS_*_mCpG.png'
        cond_list_dic[ res_dir ] = sorted( [ xxx.split( '_3SS_' )[ -1 ].split( '_mCpG.' )[ 0 ] for xxx in glob.glob( cond_list_pattern ) ] )
        pass
    ...
    # add title
    page_title_dyn = page_title + ' / ' + measure
    ...
    for cond_idx in range( cond_list_nb ):
        ...
        for res_dir_idx, res_dir in enumerate( res_dir_list ):
            ...
            current_pane_x = current_x + res_dir_idx * ( offset_x + siGL2_signal_img_height * 2 )
            glob_pattern = res_dir + '/' + feature_dir + '/' + measure + '/*_3SS_' + cond + '_mCpG.png'
            ...
            glob_pattern = res_dir + '/' + feature_dir + '/' + measure + '/*_5SS_' + cond + '_mCpG.png'
            image_file = glob.glob( glob_pattern )[ 0 ]
            img_pane_fc( image_file, slide, current_pane_x, current_y, siGL2_signal_img_height )
            pass
