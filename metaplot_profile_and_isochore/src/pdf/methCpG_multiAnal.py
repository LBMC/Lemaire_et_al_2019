...
def col_names_fc( slide, slide_marge_left, siGL2_signal_img_height, current_y, offset_x ):
    current_y = current_y + height_tbx
    current_x = slide_marge_left + siGL2_signal_img_height / 2
    for ss_str_idx, ss_str in enumerate( [ '3SS', '5SS' ] ):
        current_pane_x = current_x + ss_str_idx * ( offset_x + siGL2_signal_img_height * 2 )
...
for measure_idx, measure in enumerate( measure_list ):
    ...
    # add title
    page_title_dyn = page_title + ' / ' + measure + '/' + stat
    ...
    current_x, current_y = col_names_fc( slide, slide_marge_left, siGL2_signal_img_height, current_y, offset_x )
    ...
    for res_dir_idx, res_dir in enumerate( res_dir_list ):
        ...
        for ss_str_idx, ss_str in enumerate( [ '3SS', '5SS' ] ):
            current_pane_x = current_x + ss_str_idx * ( offset_x + siGL2_signal_img_height * 2 )
            glob_pattern = res_dir + '/' + feature_dir + '/' + measure + '/*_' + ss_str + '_*' + stat + '.png'
